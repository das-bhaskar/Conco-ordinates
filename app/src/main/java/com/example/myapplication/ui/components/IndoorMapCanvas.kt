...
private val PATH_COLOR = Color(0xFF912338)        // Concordia maroon route line
...
fun IndoorMapCanvas(
    ...
    fun autoFit() {
        ...
        val PAD    = 0.03f
        val scaleX = canvasSize.width  / ((contentW + PAD * 2) * canvasSize.width)
        val scaleY = canvasSize.height / ((contentH + PAD * 2) * canvasSize.height)
        scale = minOf(scaleX, scaleY).coerceIn(0.4f, 6f)
        val cx = (minX + maxX) / 2f; val cy = (minY + maxY) / 2f
        panOffset = Offset(
            canvasSize.width  / 2f - cx * canvasSize.width  * scale,
            canvasSize.height / 2f - cy * canvasSize.height * scale
        )
        fitted = true
    }
    ...
    Canvas(
        modifier = modifier
            ...
            .pointerInput(Unit) {
                detectTransformGestures { centroid, pan, zoom, _ ->
                    val next  = (scale * zoom).coerceIn(0.4f, 10f)
        ...
        floor.rooms.forEach { room ->
            ...
            val onPath = pathNodeIds.isNotEmpty() && floor.nodes.any { it.roomId == room.id && it.id in pathNodeIds }
            val fill   = when {
        ...
        if (pathEdgeIds.isNotEmpty()) {
            ...
            pathEdgeIds.forEach { (fromId, toId) ->
                ...
                drawLine(
                    color       = PATH_COLOR,
                    start       = toScreen(a.x, a.y),
                    end         = toScreen(b.x, b.y),
                    strokeWidth = (6f * scale).coerceIn(4f, 14f)
                )
            }
            // white dashes on top for direction feel
            pathEdgeIds.forEach { (fromId, toId) ->
                val a = nodeMap[fromId] ?: return@forEach
                val b = nodeMap[toId]   ?: return@forEach
                drawLine(
                    color       = PATH_DOT.copy(alpha = 0.55f),
                    start       = toScreen(a.x, a.y),
                    end         = toScreen(b.x, b.y),
                    strokeWidth = (2f * scale).coerceIn(1f, 5f),
                    pathEffect  = PathEffect.dashPathEffect(floatArrayOf(12f, 10f), 0f)
                )
            }
            // start dot (green) and end dot (maroon)
