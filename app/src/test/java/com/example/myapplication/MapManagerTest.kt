...
class MapManagerTest {
    ...
    @Test
    fun `test getCenterLatLng for a building without wayID`() = runTest() {
        var building = Building(
            "Henry F. Hall Building",
            "H",
            0,
    ...
    @Test
    fun `test getPanorama for false coordinates when getCenterLatLng doesn't return true`() = runTest() {
        var result = mapManager.getPanorama(LatLng(0.0, 0.0))
        assertEquals(null, result)
    }
